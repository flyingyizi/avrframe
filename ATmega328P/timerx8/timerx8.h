...
///		value of the timer can be read by accessing the CPU register \c
/// TCNT0. 		We could, for example, figure out how long it takes to
/// execute a 		C command by recording the value of \c TCNT0 before and
...
///		general-purpose timers, but T1 has special hardware allowing it
/// to 		generate PWM signals, while T2 is specially designed to help
/// count 		out real time (like hours, minutes, seconds).  See the
...
typedef enum PRESCALER_5T {
  ...
  /*#define*/ TIMER_PRESCALER5_MASK = 0x07  ///< Timer Prescaler Bit-Mask
} PRESCALER_5;

typedef enum PRESCALER_7T {
  ...
  /*#define*/ TIMERRTC_PRESCALE7_MASK = 0x07  ///< RTC Timer Prescaler Bit-Mask
} PRESCALER_7;
...
// timer0 溢出中断初始化
void timer0OVFInit(uint8_t init_value);  ///< initialize timer0
// timer1 溢出中断初始化
void timer1OVFInit(uint16_t init_value);  ///< initialize timer1
#ifdef TCNT2                              // support timer2 only if it exists
void timer2OVFInit(uint8_t init_value);   ///< initialize timer2
#endif
...
void timer0SetPrescaler(PRESCALER_5 prescale);  ///< set timer0 prescaler
uint16_t timer0GetPrescaler(void);              ///< get timer0 prescaler
void timer1SetPrescaler(PRESCALER_5 prescale);  ///< set timer1 prescaler
uint16_t timer1GetPrescaler(void);              ///< get timer0 prescaler
#ifdef TCNT2  // support timer2 only if it exists
void timer2SetPrescaler(PRESCALER_7 prescale);  ///< set timer2 prescaler
...
//
//		timerAttach causes the myOverflowFunction() to be attached, and
...
// timer1PWMInit()
//		configures the timer1 hardware for PWM mode on pins OC1A and
// OC1B. 		bitRes should be 8,9,or 10 for 8,9,or 10bit PWM
// resolution
//
// timer1PWMOff()
//		turns off all timer1 PWM output and set timer mode to normal
// state
//
// timer1PWMAOn() and timer1PWMBOn()
//		turn on output of PWM signals to OC1A or OC1B pins
//		NOTE: Until you define the OC1A and OC1B pins as outputs, and
// run 		this "on" command, no PWM output will be output
//
...
void timer1PWMASet(
    uint16_t pwmDuty);  ///< set duty of timer1 Channel A (OC1A) PWM output
void timer1PWMBSet(
    uint16_t pwmDuty);  ///< set duty of timer1 Channel B (OC1B) PWM output
